...
public class OrderServiceTest {
    ...
    public void 상품주문() {
        ...
        Book book = getBook("시골 JPA", 10, 10000);
        ...
        Assert.assertEquals("상품 주문시 상채는 ORDER", OrderStatus.ORDER, getOrder.getStatus());
    ...
    public void 상품주문_재고수량초과() throws Exception{
        //given
        Member member = getMember();
        Book book = getBook("상품주문_재고수량초과_테스트", 10, 10000);

        int orderCount =11;

        //when

        //재고수량 초과 테스트는 removeStock() 단위테스트로 하는 것이 가장 좋다.
    ...
    public void 주문취소() {
        //give
        Member member = getMember();
        Item item = getBook("시골 JPA",10,10000);
    ...
    private Book getBook(String name, int quantity, int price) {
        Book book  = new Book();
        book.setName(name);
        book.setPrice(price);
    ...
    private Member getMember() {
        ...
        member.setName("회원1");
        member.setAddress(new Address("서울","석촌호수로","284"));
